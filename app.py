...
def createPng(dictionary,isWorld):
    print dictionary
    ...
    print str(isWorld)+" is world"
    if int(isWorld)==1:
        #print "here1"
        svg= open('/home/ms/PycharmProjects/A.R.M.Y2/world.svg', 'r').read()
        #print "here"
        pr = 'data-iso3'
    else:
    ...
    #red = Color("red")
    #blue = Color("blue")
    #colour_array=list(red.range_to(blue, 5))
    #print colour_array
    ...
    for p in paths:
        ...
        if p[pr] not in ["State_Lines", "separator","path4148","MI-","SP-"]:
            #print str(p['id'])
            print p[pr]

            if str(p[pr]).startswith("path"):
                continue

            if p[pr] not in dictionary.keys():
               color = Color("blue", saturation=0)
               print "here"
            else:
               color = Color("blue", saturation= dictionary[str(p[pr])])

            ''' if int(p['id'])<10:
                color=colour_array[0]

            elif int(p['id'])<20:
                color=colour_array[1]

            elif int(p['id'])<30:
                color=colour_array[2]

            elif int(p['id'])<40:
                color=colour_array[3]

            elif int(p['id'])<50:
                color=colour_array[4]

            else:
                color=colour_array[5]'''
...
def getOutput():
    ...
    if int(_input1)==3:
        ...
        isWorld=1
        l.append('deathCountry')

    l.append(int(_range))

    print l
    data= analysis(l)
    createPng(data,isWorld)
